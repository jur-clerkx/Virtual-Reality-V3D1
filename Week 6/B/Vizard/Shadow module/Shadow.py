...
duck1 = viz.addChild('duck.cfg')
duck1.setPosition([0, 2, 0])
duck2 = viz.addChild('duck.cfg')
duck2.setPosition([0, 0, 0])

shadowProjector = ShadowProjector(pos=(0,3.5,0))

shadowProjector.addCaster(duck1)
shadowProjector.addReceiver(duck2)
shadowProjector.addReceiver(lab)
shadowProjector.visible(False)

# Perform a simple animation so light results are better noticable
startLeft = vizact.move(-2,0,0,0.5)
duck1.addAction(startLeft)
moveRight = vizact.move(4,0,0,1) 
moveLeft = vizact.move(-4,0,0,1)
moveDown = vizact.move(0,-3,0,1)
moveUp = vizact.move(0,3,0,1)
moveInSquare = vizact.sequence(moveRight,moveLeft,moveDown,moveUp,viz.FOREVER) 
duck1.addAction(moveInSquare)